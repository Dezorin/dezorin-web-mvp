...
// POST /api/explore
// المدخل: { breakdown: string[] }
// المخرج: { candidates_generated: number, qualified_relations: [{id, elements_used, what_emerged}] }
router.post('/explore', quotaMiddleware, async (req, res) => {
  const { breakdown } = req.body || {};

  if (!Array.isArray(breakdown) || breakdown.length === 0) {
    return res.status(400).json({ error: 'نفّذ مرحلة التفكيك أولًا — لا وحدات متاحة للاستكشاف.' });
  }
...
راجع هذا المرشح وفق الحكمين المحددين، وأعد النتيجة بصيغة JSON المحددة فقط.`;
      ...
      const judgeData = await callOpenAI(OPENAI_API_KEY, EXPLORATION_JUDGE_SYSTEM_PROMPT, judgeUserPrompt);
      await recordUsage(req.supabase, req.userId, 'exploration');

      const verdicts = Array.isArray(judgeData.verdicts) ? judgeData.verdicts : [];
      const v = verdicts[0];
      if (v && v.traceable_to_material === true && v.valid_inference === true) {
    ...
    return res.status(502).json({ error: 'حدث خطأ أثناء الاستكشاف: ' + err.message });
  }
});
